...
.ld-popup{{display:none;position:absolute;right:0;top:100%;z-index:100;background:#0d1520;border:1px solid #4fc3f7;border-radius:8px;padding:12px 16px;min-width:300px;max-width:90vw;width:420px;box-shadow:0 8px 32px rgba(0,0,0,.6);font-size:11px;color:#b0bec5;text-align:left;white-space:normal}}
.ld-cell:hover .ld-popup,.ld-cell.open .ld-popup{{display:block}}
...
function toggleLd(el){{var wasOpen=el.classList.contains('open');document.querySelectorAll('.ld-cell.open').forEach(function(c){{c.classList.remove('open')}});if(!wasOpen)el.classList.add('open')}}
document.addEventListener('click',function(e){{var cell=e.target.closest('.ld-cell');if(!cell)document.querySelectorAll('.ld-cell.open').forEach(function(c){{c.classList.remove('open')}})}});
