...
## Reroll detector: friendly-context keywords are false positives (follow-up filed)
"Judgement of the Omnissiah" (Space Wolves Iron Priest): "…targets an enemy unit within Engagement Range of one or more friendly ADEPTUS ASTARTES VEHICLE units, you can re-roll the wound roll." The VEHICLE there is a CONTEXT (proximity to friendly transports), not the target type — the parser naively records `targets=['VEHICLE']`, so the engine grants the reroll when the target is T≥6 rather than when the enemy is near a friendly vehicle. Both branches drift from the rule; impact localized to 1 unit per dataset. Accepted for this pass (Shredder: ship-able), filed as a follow-up: treat `within … of`/`friendly` contexts as non-target.
**How:** next pass — skip keyword mentions inside "within X of"/"friendly" phrases before adding them to `targets`; add a probe test.
