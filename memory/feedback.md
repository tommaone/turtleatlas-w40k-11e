...
## 2026-09-05 - All 28 factions rated (research sprint complete)

- Splinter-style dispatch: 5 parallel research agents (SM chapters / xenos / chaos / imperium / misc). Batch B/D/E agents returned EMPTY completion messages; resumed their sessions via task_id to force the actual verdicts out — succeeded without re-dispatching.
- Final bands: 10 Strong (TS, EC, DA, Necrons, Tau, Orks, Daemons, Custodes, Deathwatch, Votann), 9 Moderate (GK, SM, SW, WE, CSM, AdMech, Sororitas, GSC, Tyranids), 2 Situational (Drukhari, IK), 7 Weak (BA, BT, Aeldari, Agents, DG, AM, CK). All carry Sources trails inside the last-month window.
- Verified army-rule label updates (11th-ed worth): BA inherits Oath of Moment with the +1-to-wound claw VOID for keyword units; SW adds Curse of the Wulfen/Sagas; Tau rule name is "For the Greater Good" (not Observer/Guided); CK = Harbingers of Dread; DW = Oath + Kill Teams; Votann = Prioritised Efficiency; Agents = Assigned Agents (none for mono). h_army labels updated to match.
